...
correctCount = document.querySelector(".quiz-title"),
...
function loadQuestion(questionNum) {
    if(currentQuestion === questions.length) {
        ...
        correctCount.innerHTML = `Correct: ${correct}/${question.length}`;
        correct = 0;
        currentQuestion = 0;
    } else {
        ...
        questionElement.innerHTML = question[questionNum].text;

        // Question Type
        if(question[questionNum].type === "mc") {
            var btnGrid = document.createElement("div");
            answersContainer.appendChild(btnGrid);
            questions[questionNum].answers.forEach((answer) => {
                var answerElement = document.createElement("button");
                btnGrid.className.add("btn-grid");
            ...
            answersContainer.dataset.type = "mc";
        } else if (question[questionNum].type === "text") {
                ...
                answersContainer.dataset.type = "text";
            }
        ...
        function checkAnswer(){
            ...
            switch(answersContainer.dataset.type){
                case "mc":
                    ...
                    var foundValues = question[currentQuestion].answer.find(
                        (answer) => answer.topUpperCase() === qInputElement.ariaValueMax.toUpperCase()
                    );
                    if(foundValues) {
